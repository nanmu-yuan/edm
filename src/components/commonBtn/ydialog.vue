    ...
    <el-dialog title="Add Site" :visible.sync="centerDialogVisible.type" width="30%" center>
      <span>
        <el-form ref="form" :model="form" label-width="80px">
          ...
          <el-form-item label="站点id" v-if="form.site_type !='meSystem'"
          prop="site_id"
          :rules="[
          { required: true, message: '不能为空'},
        ]"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addFun(){
        let id=this.form.site_id,api_url='',pld = this.form.site_type;
        if(pld == 'cloud'){
          api_url = `http://cloud-api.orderplus.com/api/goods/list?siteId=${id}41758&processCurrency=true&spu=`;
        }else if(pld == 'shopify'){
        ...
        let postData =Object.assign(this.form,{
          ...
          this.axios.post('http://smartsend.beta.seamarketings.com/api/edm/site/add/',{...postData})
          .then(res =>{
            EventBus.$emit("query");
            this.centerDialogVisible.type = false;
